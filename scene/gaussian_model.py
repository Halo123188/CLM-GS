    ...
    def load_tensors(
        ...
        if self.args.offload:
            if self.args.braindeath_offload:
                ...
                self._scaling = nn.Parameter(_scaling.pin_memory()).requires_grad_(True)
                self._rotation = nn.Parameter(_rotation.pin_memory()).requires_grad_(True)
                self._parameters = nn.Parameter(_features.pin_memory()).requires_grad_(True)
                self._features_dc, self._features_rest = torch.split(self._parameters, [3, 45], dim=1)

            # if self.args.braindeath_offload or self.args.gpu_cache == "no_cache":
            #     self._xyz = nn.Parameter(_xyz.pin_memory()).requires_grad_(True)
            #     self._opacity = nn.Parameter(_opacity.pin_memory()).requires_grad_(True)
            #     self._scaling = nn.Parameter(_scaling.pin_memory()).requires_grad_(True)
            #     self._rotation = nn.Parameter(_rotation.pin_memory()).requires_grad_(True)
            ...
            #     self._features_dc, self._features_rest = torch.split(self._parameters, [3, 45], dim=1)
                ...
                assert self.args.gpu_cache == "xyzosr"
                parameters_buffer_array = numba.cuda.pinned_array((self.args.prealloc_capacity, 48), dtype=np.float32)
                self.parameters_buffer = torch.from_numpy(parameters_buffer_array)
                parameters_grad_buffer_array = numba.cuda.pinned_array((self.args.prealloc_capacity, 48), dtype=np.float32)
                self.parameters_grad_buffer = torch.from_numpy(parameters_grad_buffer_array)
            ...
            self._scaling = nn.Parameter(_scaling.to("cuda")).requires_grad_(True)
            self._rotation = nn.Parameter(_rotation.to("cuda")).requires_grad_(True)
            self._features_dc = nn.Parameter(_features[:, :3].reshape(-1, 1, 3).to("cuda")).requires_grad_(True)
            self._features_rest = nn.Parameter(_features[:, 3:].reshape(-1, 15, 3).to("cuda")).requires_grad_(True)
    ...
    def distributed_load_ply(self, folder):
        ...
        if self.args.braindeath_offload:
            ...
            if self.args.gpu_cache == "no_cache":
                ...
                self.parameters_buffer[:N] = torch.cat([_xyz, _opacity, _scaling, _rotation, _features_dc, _features_rest], dim=1)
                self._parameters = nn.Parameter(
                    self.parameters_buffer[:N].requires_grad_(True)
                )
                dims = [_xyz.shape[1], _opacity.shape[1], _scaling.shape[1], _rotation.shape[1], _features_dc.shape[1], _features_rest.shape[1]]
                self._xyz, self._opacity, self._scaling, self._rotation, self._features_dc, self._features_rest = torch.split(self._parameters, dims, dim=1)
                self.param_dims = torch.tensor(dims, dtype=torch.int, device='cuda')
                self.param_dims_presum_rshift = torch.cumsum(self.param_dims, dtype=torch.int, dim=0) - self.param_dims
                self.col2attr = torch.empty((sum(dims),), dtype=torch.int, device='cuda')
                ...
                if args.reinit_ply:
                    ...
                    dist2 = torch.clamp_min(
                        distCUDA2(torch.from_numpy(_xyz).float().cuda()),
                        0.0000001,
                    )
                    scales = torch.log(torch.sqrt(dist2))[..., None].repeat(1, 3).to("cuda")
                    ...
                    # self.parameters_buffer[:N] = torch.tensor(np.concatenate((_features_dc, _features_rest), axis=1))
                    self._parameters = nn.Parameter(
                        self.parameters_buffer[:N].requires_grad_(True)
                    )
                    self._features_dc, self._features_rest = torch.split(self._parameters, [3, 45], dim=1) 
                    self._opacity = nn.Parameter(
                        _opacity.to(torch.float).to("cuda").requires_grad_(True)
                    )
                    self._scaling = nn.Parameter(
                        _scaling.to(torch.float).to("cuda").requires_grad_(True)
                    )
                    self._rotation = nn.Parameter(
                        _rotation.to(torch.float).to("cuda").requires_grad_(True)
                    )
                self.max_radii2D = torch.zeros((self.get_xyz.shape[0]), device="cuda")
                self.sum_visible_count_in_one_batch = torch.zeros(
            ...
            self._features_dc = nn.Parameter(
                _features_dc.to(torch.float).to("cuda")
                .transpose(1, 2)
                .contiguous()
                .requires_grad_(True)
            )
            ...
            if args.reinit_ply:
                self._features_rest = nn.Parameter(
                    _features_rest.to(torch.float).to("cuda")
                    .transpose(1, 2)
                    .contiguous()
                    .zero_()
                    .requires_grad_(True)
