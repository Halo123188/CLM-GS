...
class GaussianModelXYZOSROffload(BaseGaussianModel):
    ...
    def distributed_load_ply(self, folder):
        ...
        # Handle upsampling/subsampling (same logic as one_file_load_ply)
        ...
        if args.reinit_ply:
            ...
            self._xyz = nn.Parameter(_xyz.to(torch.float).to("cuda").requires_grad_(True))
            self._scaling = nn.Parameter(scales.requires_grad_(True))
            self._rotation = nn.Parameter(rots.requires_grad_(True))
            self._opacity = nn.Parameter(opacities.requires_grad_(True))
            ...
            self._xyz = nn.Parameter(_xyz.to(torch.float).to(self.device).requires_grad_(True))
            self._opacity = nn.Parameter(_opacity.to(torch.float).to(self.device).requires_grad_(True))
            self._scaling = nn.Parameter(_scaling.to(torch.float).to(self.device).requires_grad_(True))
            self._rotation = nn.Parameter(_rotation.to(torch.float).to(self.device).requires_grad_(True))
