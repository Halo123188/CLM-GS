...
from strategies.naive_offload import (
...
def generate_convex_hull_trajectory_v2(
    ...
    hull_points = np.array(
        [  # (cam-to-world transformation) world coordinates
            # [-20, 20, height_z],
            [0, 35, height_z],
            [10, 30, height_z],
            [15, 20, height_z],
    ...
    utils.print_rank_0(
        f"  Generated {len(trajectory_cameras)} cameras along convex hull"
    )
...
def visualize_point_cloud_projection(
    ...
    if camera_trajectory is not None and len(camera_trajectory) > 0:
        ...
        for cam in camera_trajectory:
            ...
            if hasattr(cam, "T") and hasattr(cam, "R"):
                ...
                T = (
                    cam.T if isinstance(cam.T, np.ndarray) else cam.T
                )  # R and T are world-to-camera
    ...
    if camera_trajectory is not None and len(camera_trajectory) > 0:
        ...
        for artist in (
            ax.lines + ax.collections[1:] + ax.patches
        ):  # Keep the first collection (scatter plot of points)
            artist.remove()
        ...
        print(
            f"Point cloud visualization (no trajectory) saved to: {output_path_no_traj}"
        )
        ...
        if "cam_positions" in locals() and len(cam_positions) > 0:
            try:
                ...
                n_frames_video = max(
                    n_frames_video, 30
                )  # At least 30 frames for smooth video

                # Calculate step size to cover entire trajectory
                step_size = len(cam_positions) / n_frames_video

                print(
                    f"Rendering {n_frames_video} video frames covering all {len(cam_positions)} trajectory positions..."
                )
                ...
                for frame_idx in range(n_frames_video):
                    ...
                    outline_draw.polygon(
                        poly_points, outline=(255, 255, 255, 120), width=1
                    )
                    ...
                    r_glow = 15
                    draw.ellipse(
                        (px - r_glow, py - r_glow, px + r_glow, py + r_glow),
                    ...
                    if frame_idx % 10 == 0:
                        print(
                            f"Rendering video frame {frame_idx}/{n_frames_video} (trajectory position {traj_idx}/{len(cam_positions)})",
                            end="\r",
                        )
...
def main():
    ...
    parser.add_argument(
        "--save_video", action="store_true", help="Also save rendered images as a video"
    )
    parser.add_argument(
    ...
    if args.save_video:
        video_path = os.path.join(
            args.output_dir,
            f"trajectory_{args.traj_path}_{args.manual_height}m_{args.n_frames}frames.mp4",
        )
        writer = imageio.get_writer(video_path, fps=30)
