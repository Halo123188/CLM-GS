...
from strategies.clm_offload import (
...
def training(dataset_args, opt_args, pipe_args, args, log_file):
    ...
    for iteration in range(
        ...
        batched_world_view_transform_inverse = torch.inverse(
            batched_world_view_transform
        )
            ...
            if (
                ...
                if not args.stop_update_param:
                    ...
                    if args.sparse_adam:
                        ...
                        if args.naive_offload:
                            sparse_indices = (
                                torch.nonzero(visibility.squeeze())
                                .flatten()
                                .to(torch.int32)
                            )
                            sparse_indices = sparse_indices.to("cpu")
                            gaussians.optimizer.sparse_step(
    ...
    if args.log_cpu_adam_trailing_overhead:
        ...
        average_cpu_adam_trailing_from_default_stream = args.cpu_adam_trailing_overhead[
            "from_default_stream"
        ] / (args.cpu_adam_trailing_overhead["step"] - 3)
        average_cpu_adam_trailing_from_comm_stream = args.cpu_adam_trailing_overhead[
            "from_comm_stream"
        ] / (args.cpu_adam_trailing_overhead["step"] - 3)
                ...
                for camera_id, (image, gt_camera) in enumerate(
                    ...
                    if idx < args.num_save_images_during_eval:
                        ...
                        torchvision.utils.save_image(
                            image, os.path.join(save_dir, render_filename)
                        )

                        # Save ground-truth image
                        torchvision.utils.save_image(
                            gt_image, os.path.join(save_dir, gt_filename)
                        )
