...
def clm_offload_train_one_batch(
    ...
    for micro_idx in range(num_micro_batches):
        ...
        filtered_xyz_gpu = torch.gather(
            gaussians._xyz.detach(), 0, this_filter.reshape(-1, 1).expand(-1, 3)
        ).requires_grad_(True)
        _filtered_opacity_gpu = torch.gather(
            gaussians._opacity.detach(), 0, this_filter.reshape(-1, 1)
        ).requires_grad_(True)
        _filtered_scaling_gpu = torch.gather(
            gaussians._scaling.detach(), 0, this_filter.reshape(-1, 1).expand(-1, 3)
        ).requires_grad_(True)
        _filtered_rotation_gpu = torch.gather(
            gaussians._rotation.detach(), 0, this_filter.reshape(-1, 1).expand(-1, 4)
        ).requires_grad_(True)
        ...
        with torch.no_grad():
            ...
            gaussians._opacity.grad.scatter_add_(
                dim=0, src=_filtered_opacity_gpu.grad, index=this_filter.reshape(-1, 1)
            )
    ...
    assert (
        not args.stop_update_param
    ), "Overlap CPUAdam does not support stop_update_param"
    ...
    if args.log_cpu_adam_trailing_overhead:
        ...
        if args.cpu_adam_trailing_overhead["step"] > 3:  # Skip first 3 warmup steps
            args.cpu_adam_trailing_overhead[
                "from_default_stream"
            ] += cpu_adam_trail_default_start_event.elapsed_time(
                cpu_adam_trail_default_end_event
            )
            args.cpu_adam_trailing_overhead[
                "from_comm_stream"
            ] += cpu_adam_trail_comm_start_event.elapsed_time(
                cpu_adam_trail_comm_end_event
            )
