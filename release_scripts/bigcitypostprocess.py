...
def create_psnr_table_from_csv(csv_path, output_md=None):
    ...
    oom_rows = [
        {"experiment_id": "bigcity_102M_naive_offload"},
        {"experiment_id": "bigcity_102M_no_offload"},
        {"experiment_id": "bigcity_25M_no_offload"},
    ]
    ...
    if "num_3dgs" in result_df.columns:
        result_df["num_3dgs"] = pd.to_numeric(
            result_df["num_3dgs"], errors="coerce"
        ).round(0)
        result_df["num_3dgs"] = result_df["num_3dgs"].fillna("OOM")
        # Convert back to int where possible
        result_df["num_3dgs"] = result_df["num_3dgs"].apply(
            lambda x: int(x) if x != "OOM" else x
        )
    ...
    with open(output_md, "w") as f:
        f.write("# Big City Experiment Results\n\n")
    ...
    print("\n" + "=" * 80)
    print("Big City Experiment Results")
    print("=" * 80)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: python bigcitypostprocess.py <csv_path> [output_md]")
        print("  csv_path: Path to experiment_results.csv file")
        print("  output_md: (optional) Path to output markdown file")
        print("\nExample:")
        print("  python bigcitypostprocess.py experiment_results.csv")
        print("  python bigcitypostprocess.py experiment_results.csv results_table.md")
        sys.exit(1)
